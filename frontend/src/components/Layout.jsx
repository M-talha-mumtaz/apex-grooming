    ...
    <div className="min-h-screen flex flex-col">
      <header className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-zinc-950/90 backdrop-blur-md py-4 border-b border-gold/20' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-8 items-center">
            ...
            <button onClick={toggleLang} className="text-zinc-300 hover:text-gold transition-colors flex items-center">
              <Globe size={18} className="mr-1" /> {i18n.language.toUpperCase()}
            </button>
          </nav>

          {/* Mobile Menu Button */}
          <div className="md:hidden flex items-center space-x-4">
            <button onClick={toggleLang} className="text-zinc-300 hover:text-gold">
              <Globe size={20} />
            </button>
      ...
      <footer className="bg-zinc-900 border-t border-gold/20 py-12 mt-20">
        <div className="container mx-auto px-6 md:px-12 text-center text-zinc-400">
          <h2 className="text-2xl text-gold font-serif mb-6 tracking-widest uppercase">Lumina</h2>
          ...
          <p className="mt-8 text-xs opacity-50">&copy; {new Date().getFullYear()} Lumina Salon. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
